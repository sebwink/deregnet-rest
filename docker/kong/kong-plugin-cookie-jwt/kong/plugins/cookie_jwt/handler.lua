...
local jwt_decoder = require "kong.plugins.cookie_jwt.jwt_parser"
...
local function load_credential(jwt_secret_key)
  local row, err = kong.db.cookie_jwt_secrets:select_by_key(jwt_secret_key)
  if err then
    return nil, err
  end
  return row
...
local function do_authentication(conf)
  ...
  -- Retrieve the secret
  local jwt_secret_cache_key = kong.db.cookie_jwt_secrets:cache_key(jwt_secret_key)
